...
?>
    ...
    async function fetchPosts() {
        try {
            const response = await fetch('../../Controller/Post/Welcome/post.php');
            if (!response.ok) {
            ...
            allPosts.forEach(post => {
                ...
                // Row for actions (likes, comment, report)
                const actionsRow = document.createElement('tr');

                const likesCell = document.createElement('td');
                const likesText = document.createElement('p');
                likesText.style.color = '#f1f1f1';
                likesText.textContent = `Likes: ${post.likes}`;
                likesCell.appendChild(likesText);

                const likeButton = document.createElement('a');
                likeButton.href = `../../Controller/Post/PostCheck.php?post_id=${post.post_id}&action=like`;
